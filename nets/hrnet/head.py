...
class ClsHead():
  ...
  def forward(self, inputs):
    ...
    for i in range(self.num_branches):
      if i < self.num_branches - 1:
        ...
        out = self.__downsample_and_add(trans_features[i], 
                                        trans_features[i + 1], 
                                        num_channels[i + 1], 
                                        scope)
    ...
    # final layer
    final_output = conv1x1_block(out, 
                                 self.final_layer_num, 
                                 scope=self.scope + '_final', 
                                 has_relu=True)
    ...
    with tf.variable_scope(self.scope):
      ...
      # fc
      output = slim.fully_connected(inputs=output, units=self.cls_num)
  ...
  def __make_trans_layers(self, inputs, num_channels):
    ...
    for i, _input in enumerate(inputs):
      _output = bottleneck_block(_input, 
                                 num_channels[i], 
                                 scope=self.scope + '_BK' + str(i), 
