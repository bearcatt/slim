...
class HRModule():
  ...
  def forward(self, inputs):
    ...
    for i, _input in enumerate(inputs):
      _output_per_resolution = self.build_network_per_resolution(
        _input, channels=self.num_channels * pow(2, i), scope=self.scope + '_B' + str(i))
  ...
  def __get_dwsample_features(self, features):
    ...
    dwmatrix = create_downsample_fn_matrix(
      self.num_branches, features, self.num_branches, output_channels)
  ...
  def __get_upsample_features(self, features):
    ...
    # create downsample fn matrix
    upmatrix = create_upsample_fn_matrix(
      self.num_branches, features, self.num_outputs, output_channels)
    ...
    for key, value in upmatrix.items():
      ...
      _input = value['input']
      _channels = value['outchannel']
      _out = self.upfn(_input, 2 ** (src - dst), _channels, 
                       self.scope + '_U' + str(src) + 'to' + str(dst))
  ...
  def fuse_multi_resolution(self, features):
    ...
    if self.multi_scale_output:
      ...
      output = self.downfn(input, 
                           planes=self.num_channels * (2 ** (self.num_outputs - 1)), 
                           scope=self.scope + '_D_TAIL' + str(self.num_outputs), 
                           has_relu=True)
